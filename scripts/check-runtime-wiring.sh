...
# require_non_empty checks that the environment variable named by its single argument is set and not empty; if the variable is empty it prints an error to stderr and returns 1.
require_non_empty() {
  local name="$1"
  if [[ -z "${!name:-}" ]]; then
...
# require_url verifies that the named environment variable is non-empty and its value begins with "http://" or "https://"; it prints an error to stderr and returns status 1 on failure.
require_url() {
  local name="$1"
