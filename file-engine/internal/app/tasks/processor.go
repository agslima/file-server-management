...
// NewProcessorWithStorage creates a Processor that uses the provided storage and an empty idempotency cache.
// The returned Processor has its storage set to st and initializes seenKeys for duplicate task tracking.
func NewProcessorWithStorage(st storage.Storage) *Processor {
	return &Processor{st: st, seenKeys: map[string]struct{}{}}
}

// NewProcessorWithMutationExecutor creates a Processor configured with the provided storage
// and MutationExecutor and initializes the processor's idempotency key set.
...
// NewProcessor panics with the message "use NewProcessorWithStorage" and should not be used.
// It is a removed compatibility constructor; create a local storage adapter and call NewProcessorWithStorage instead.
