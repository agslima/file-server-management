...
(
	...
	WHERE ae.metadata->>'tenant_id' = ut.tenant_id
	  AND ae.metadata->>'actor_id' = ut.user_id
	  AND (
		ae.event_type = 'object.download'
		OR ae.event_type = 'object.list'
	  )
) AS last_read_at,
(
	SELECT MAX(ae.created_at) FROM audit_events ae
	WHERE ae.metadata->>'tenant_id' = ut.tenant_id
	  AND ae.metadata->>'actor_id' = ut.user_id
	  AND (
		ae.event_type LIKE 'upload.%'
		OR ae.event_type LIKE 'folder.mutation.%'
		OR ae.event_type IN ('object.upload', 'object.delete', 'object.move', 'object.rename', 'object.restore')
	  )
) AS last_write_at,
(
	SELECT MAX(ae.created_at) FROM audit_events ae
...
ORDER BY ut.tenant_id ASC, ut.user_id ASC, utr.role_id ASC`, tenantID)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.TenantID, &r.UserID, &r.Email, &r.RoleID, &r.LastReadAt, &r.LastWriteAt, &r.LastAccess); err != nil {
