...
1. **README claims map clearly to ledger evidence.**
   - The README implementation table tracks the promoted claim set in `docs/capability-ledger.md` (currently through `CL-051`, including CI-gated deterministic OIDC evidence, upload API contract promotion evidence, scanner/upload operational closure evidence, observability assets, OTEL production deployment drills with paging delivery validation, and storage parity controls).
...
4. **Target-state boundaries remain explicit.**
   - Upload pipeline operational closure for thresholds/on-call/escalation is promoted via `CL-051`; OTEL production deployment hardening is promoted via `CL-048`, paging-provider delivery via `CL-050`, and immutable sink delivery remains promoted via `CL-035`.
