...
| Audit | Baseline task audit events are emitted for async/read flows and append-only table enforcement is baseline-validated (CL-032); immutable external sink integration remains target-state. | @agslima (Agnaldo Silva Lima) | Security reviewer rotation (`docs/security-reviewers.md`) | `cd file-engine && go test ./tests/integration -run TestAsyncCreateFolderFlow -v` and `cd file-engine && go test ./tests/integration -run TestAuditEventsAppendOnlyEnforced -v` | Phase 2 baseline; Phase 3 for sink |
